...
var { map, share, filter, takeUntil, bufferCount } = require('rxjs/operators')
...
class CheckBoxPlus extends Base {
        ...
        this.opt.source = (_, input) => {
          ...
          return new Promise(resolve => {
            setTimeout(() => {
              resolve(
                fuzzy
                  .filter(input, this.opt.autocomplete)
                  .map(el => el.original),
              )
            })
          })
    ...
    choices.forEach(function(choice, index) {
      ...
      if (index - separatorOffset === pointer) {
        ...
        if (!highlight) {
          ...
          output +=
            highlight &&
            (_.isFunction(highlight)
              ? highlight(choice.name)
  ...
  render(error, enabledSubSource) {
    ...
    if (this.searching) {
      message +=
        '\n  ' +
        (this.opt.searching ? this.opt.searching : chalk.cyan('Searching...'))

      // No choices
    } else if (!this.choices.length) {
      message +=
        '\n  ' +
        (this.opt.noresult ? this.opt.noresult : chalk.yellow('No results...'))
      ...
      // add
      var realChoiceStr = this.paginator.paginate(
        choicesStr,
        indexPosition,
        this.opt.pageSize,
      )
  ...
  endKey() {
    return [
      filter(
        ({ key }) => key.sequence === 'G' || (key.sequence === 'G' && key.ctrl),
      ),
      share(),
    ]
  }
