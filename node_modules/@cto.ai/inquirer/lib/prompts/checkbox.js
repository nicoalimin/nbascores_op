...
class CheckboxPrompt extends Base {
  ...
  _run(cb) {
    ...
    events.hKey
      .pipe(takeUntil(validation.success))
      .forEach(this.onHelpKey.bind(this));
  ...
  render(error) {
    ...
    // help message now persists after selection
    message +=
      '(Press ' +
  ...
  onAllKey() {
    var shouldBeChecked = Boolean(
      this.opt.choices.find(function (choice) {
        return choice.type !== 'separator' && !choice.checked && !choice.mandatory;
      })
    );

    this.opt.choices.forEach(function (choice) {
      if (choice.type !== 'separator' && !choice.mandatory) {
  ...
  toggleHelp(index) {
    ...
    if (item !== undefined) {
      this.opt.choices.getChoice(index).help = !item.help
    }
  }

  toggleChoice(index) {
    var item = this.opt.choices.getChoice(index);
    if (item !== undefined && !item.mandatory) {
...
function renderChoices(choices, pointer) {
  ...
  choices.forEach(function (choice, i) {
    ...
    if (choice.disabled) {
      ...
      var displayText = (choice.help) ? `${choice.name}\n${choice.helpInfo}` : choice.name
      var line = getCheckbox(choice.checked) + ' ' + displayText;
