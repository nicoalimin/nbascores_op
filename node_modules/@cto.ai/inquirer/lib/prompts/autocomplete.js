...
class AutocompletePrompt extends Base {
  constructor(
    ...
    if (!this.opt.source) {
      if (this.opt.autocomplete) {
        var arrayList = (typeof this.opt.autocomplete[0] === 'object') ? this.opt.autocomplete.map(el => el.name) : this.opt.autocomplete
        this.opt.source = (_, input) => {
          input = input || ''
          return new Promise(resolve => {
            setTimeout(() => {
              resolve(
                fuzzy
                .filter(input, arrayList)
                .map(el => {
                  return (typeof this.opt.autocomplete[0] === 'object') ?
                    this.opt.autocomplete.find(elm => elm.name === el.original) : el.original
                }),
              )
            })
          })
  ...
  onKeypress(e /* : {key: { name: string }, value: string} */ ) {
    ...
    if (keyName === 'tab' && this.opt.suggestOnly) {
      ...
      var previousSelection = this.currentChoices.getChoice(this.selected)
      if (previousSelection && previousSelection.help) previousSelection.help = false
      ...
      this.toggleHelp(this.selected) // toggles helpinfo for selected choice
      this.render();
    } else {
  ...
  toggleHelp(index) {
    var item = this.currentChoices.getChoice(index);
    if (item) {
      this.currentChoices.getChoice(index).help = !item.help
    }
  }
}
...
function listRender(choices, pointer /*: string */ ) /*: string */ {
  ...
  choices.forEach(function (choice, i) {
    ...
    var displayText = (choice.help && choice.helpInfo) ? `${choice.name}\n   ${choice.helpInfo}` : choice.name
    var isSelected = i - separatorOffset === pointer;
    var line = (isSelected ? chalk.green(figures.arrowRight) + ' ' : '  ') + displayText;
