...
declare namespace inquirer {
  ...
  interface QuestionCommon<A> {
    ...
    /**
     * Custom parameters meant for the @cto.ai/ops CLI
     */
    afterMessage?: string
  ...
  interface CheckboxPlusQuestion<A>
    extends QuestionCommon<A>,
      Pick<
        QuestionOptions<A>,
        'choices' | 'filter' | 'transformer' | 'pageSize'
      > {
    type: 'checkbox-plus'
    searchable?: boolean
    highlight?: boolean
    ...
    source?(answersSoFar: any[], input: string): any
    autocomplete?: string[] | { name: string; helpInfo: string }[]
  }

  interface EmojiQuestion<A> extends QuestionCommon<A>, QuestionOptions<A> {
    type: 'emoji'
  }

  interface FuzzyPathQuestion<A> extends QuestionCommon<A>, QuestionOptions<A> {
    type: 'fuzzypath'
  ...
  type cmbObj = { [key: string]: string }
  // based on https://github.com/mokkabonna/inquirer-autocomplete-prompt
  interface AutoCompleteQuestion<A>
    extends QuestionCommon<A>,
      Pick<QuestionOptions<A>, 'pageSize' | 'filter'> {
    type: 'autocomplete'
    source?: (
      _: A,
      input: string,
    ) => Promise<
      SourceResult[] | string[] | { name: string; helpInfo: string }[]
    >
    autocomplete?: string[] | { name: string; helpInfo: string }[]
    suggestOnly?: boolean
  ...
  // based on: https://github.com/DerekTBrown/inquirer-datepicker-prompt
  interface DatePickerQuestion<A>
    extends QuestionCommon<A>,
      Pick<QuestionOptions<A>, 'pageSize' | 'filter'> {
    type: 'datepicker'

    format?: Array<string | Date>
    initial?: Date
    date?: {
      min?: string
      max?: string
